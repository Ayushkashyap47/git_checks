...
class LinkedList:
    ...
    def delete_song(self, song_name):
        ...
        # Searching for the song to be deleted
        while temp_node is not None and temp_node.song.song_name != song_name:
            prev_node = temp_node
            temp_node = temp_node.next
        ...
        # Updating the references to remove the song from the linked list
        ...
        self.count -= 1
        print(f"Song '{song_name}' deleted from the playlist.")
        return True

    def sort_list(self):
        sorted_list = sorted(self.to_list(), key=lambda x: x.song_name, reverse=True)
        self.head_node = None
        for song in sorted_list:
            self.insert_at_start(ListNode(song))
        print("Playlist sorted in ascending order based on song names.")
...
if __name__ == "__main__":
    ...
    musicplayer.play_playlist('Songs')
#ayush_new_change_to_see_if_conflicts
